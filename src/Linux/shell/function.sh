...
_codex_switch_http_port() {
  local cfg="${XDG_CONFIG_HOME:-$HOME/.config}/codex-switch/clash-proxy.json"
  local port=""

  if [ -f "$cfg" ]; then
    port=$(sed -n 's/.*"httpPort"[[:space:]]*:[[:space:]]*\([0-9][0-9]*\).*/\1/p' "$cfg" | head -n 1)
  fi
  if [ -z "$port" ]; then
    port=1990
  fi
...
_codex_switch_use_proxy() {
  local port
  port=$(_codex_switch_http_port)
  if ! _proxy_check_port "127.0.0.1" "$port"; then
    echo "error: codex-switch 没有在 127.0.0.1:${port} 监听。先运行 codex-switch 并启动。" >&2
    return 1
  fi
  export http_proxy="http://127.0.0.1:${port}"
  export https_proxy="http://127.0.0.1:${port}"
...
gemini() {
  _codex_switch_use_proxy || return 1
  command gemini "$@"
}
...
select_fastest_relay() {
  ...
  local result nodes current best_node best_delay encoded_group encoded_test_proxy payload delay proxies_result dialer_proxy limiter_mode
  ...
  if [ -z "$dialer_proxy" ]; then
    limiter_mode=1
    timeout="${SELECT_FASTEST_RELAY_TIMEOUT:-8000}"
    api_timeout="${SELECT_FASTEST_RELAY_API_TIMEOUT:-12}"
    echo "[...] limiter mode: $test_proxy has no dialer-proxy (local SOCKS); measuring full chain via $group"
  elif [ "$dialer_proxy" != "$group" ]; then
    echo "error: $test_proxy dialer-proxy mismatch" >&2
    echo "  expected: $group" >&2
    echo "  actual: ${dialer_proxy:-empty}" >&2
    echo "hint: full-chain latency requires dialer-proxy=$group, or a local limiter with empty dialer-proxy" >&2
    _select_fastest_relay_print_hints "$proxies_result"
    return 1
  fi
